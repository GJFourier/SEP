...
Loadgamescene::Loadgamescene(QWidget *parent) : QMainWindow(parent)
{
    //设置界面大小
    ...
    back_btn->show();
    single_gamescene = new Singlegamescene;

    //实现模式选择
    connect(single_btn,&QPushButton::clicked,[=](){
        single_gamescene->setGeometry(this->geometry());
        single_gamescene->show();
        this->hide();
    });
    connect(single_btn,&QPushButton::clicked,single_gamescene,&Singlegamescene::timestart);
    connect(single_btn,&QPushButton::clicked,single_gamescene,&Singlegamescene::setScore);
    connect(single_btn,&QPushButton::clicked,single_gamescene,&Singlegamescene::mapInit);
}
