...
void Lexicon::addWordsFromFile(const std::string &filename) {
  ...
  if (istr.fail()) {
    throw new std::runtime_error(
        "Lexicon::addWordsFromFile: Couldn't open lexicon file " + filename);
  }
  std::string line;
...
bool Lexicon::remove(const std::string &word) {
  ...
  return removeHelper(m_root, scrubbed, /* originalWord */ scrubbed,
                      /* isPrefix */ false);
}
...
int Lexicon::size() const { return m_size; }
...
bool Lexicon::removeHelper(TrieNode *&node, const std::string &word,
  ...
  if (node == nullptr) {
    ...
    // recursive case: chop off first letter, traverse the rest
    return removeHelper(node->child(word[0]), word.substr(1), originalWord,
                        isPrefix);
  }
}
...
static bool scrub(std::string &str) {
  ...
  for (size_t i = 0; i < nChars; i++) {
    std::string::value_type ch =
        static_cast<std::string::value_type>(tolower(str[i]));
    if (ch < 'a' || ch > 'z') {
